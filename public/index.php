...
Flight::route('GET /trajet/reserver/@id', function($id){
    ...
    // Vérifier si l'utilisateur a déjà une réservation ACTIVE pour ce trajet
    $sqlCheck = "SELECT * FROM RESERVATIONS 
                WHERE id_trajet = :id 
                AND id_passager = :user 
                AND statut_code IN ('V', 'P')"; // On exclut les annulées ('A')
    ...
    foreach ($reservations as &$reservation) {
        ...
        if (isset($reservation['duree_estimee'])) {
            ...
            if ($heures > 0 && $minutes > 0) {
                $reservation['duree_fmt'] = $heures . "h" . $minutes;
            } elseif ($heures > 0) {
                $reservation['duree_fmt'] = $heures . " heure" . ($heures > 1 ? "s" : "");
            } else {
                $reservation['duree_fmt'] = $minutes . " minutes";
            }
        } else {
...
Flight::route('POST /reservation/annuler/@id', function($id){
    ...
    try {
        ...
        // Mettre à jour le statut à 'A' (Annulée)
